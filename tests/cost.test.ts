...
describe('Cost Estimator', () => {
  ...
  describe('calculateModelTokens', () => {
    ...
    it('should apply model-specific multipliers', () => {
      ...
      expect(calculateModelTokens(prompt, 'gpt-4')).toBe(
        Math.ceil(baseTokens * 1.1)
      );

      // GPT-3.5-turbo multiplier is 1.05
      expect(calculateModelTokens(prompt, 'gpt-3.5-turbo')).toBe(
        Math.ceil(baseTokens * 1.05)
      );
  ...
  describe('estimateCost', () => {
    ...
    it('should return a finite non-negative cost', () => {
      const prompt =
        'This is a longer test prompt to get more precise cost calculation';
      const result = estimateCost(prompt, 'gpt-4');
